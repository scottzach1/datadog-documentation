...
4. Add the `mcpServers` block below. If the file already has content (e.g. a `"preferences"` key), merge the `mcpServers` key into the existing JSON rather than replacing the file.
    ...
    - Linux (replace `<USERNAME>` with your OS username):
       ...
       **Tip:** Run `which datadog_mcp_cli` to confirm the exact path.
